<?php
	require_once("../bo/CategoriaBO.php");
	require_once("../bo/ModuloBO.php");
	session_start();
	session_regenerate_id();
	$usuarioLogueado = isset($_SESSION['idUsuario']) && isset($_SESSION['nickUsuario']) && $_SESSION['idUsuario']!= "" && $_SESSION['nickUsuario']!= ""? true: false;
	if($usuarioLogueado){
?>
<html>
	<head>
		<meta http-equiv="Content-Type" content="text/html; charset=ISO 8859-1" />
		<title>Blog</title>
		<link  rel="shortcut icon" href="../img/favicon.ico"/>
		<link type = "text/css" rel = "stylesheet" href = "../libs/jPaginate/css/style.css"/>
		<link type = "text/css" rel = "stylesheet" href = "../css/estilos.css"/>
		<script type = "text/javascript" src="../libs/jquery/jquery-1.6.2.min.js"> </script>
		<script type = "text/javascript" src="../libs/jPaginate/jquery.paginate.js"> </script>
		<script type = "text/javascript" src="../libs/jquery-ui-1.8.15.custom/development-bundle/ui/minified/jquery.effects.core.min.js"></script>
		<script type = "text/javascript" src="../libs/jquery-ui-1.8.15.custom/development-bundle/ui/minified/jquery.effects.bounce.min.js"></script>
		<script type = "text/javascript" src="../libs/jquery-ui-1.8.15.custom/development-bundle/ui/minified/jquery.effects.pulsate.min.js"></script>
		<!--
		<script src="../libs/jquery-ui-1.8.15.custom/development-bundle/ui/minified/jquery.effects.drop.min.js"></script>
		<script src="../libs/jquery-ui-1.8.15.custom/development-bundle/ui/minified/jquery.effects.slide.min.js"></script>
		<script src="../libs/jquery-ui-1.8.15.custom/development-bundle/ui/minified/jquery.effects.blind.min.js"></script>
		<script src="../libs/jquery-ui-1.8.15.custom/development-bundle/ui/minified/jquery.effects.clip.min.js"></script>
		<script src="../libs/jquery-ui-1.8.15.custom/development-bundle/ui/minified/jquery.effects.fold.min.js"></script>
		<script src="../libs/jquery-ui-1.8.15.custom/development-bundle/ui/minified/jquery.effects.highlight.min.js"></script>
		<script src="../libs/jquery-ui-1.8.15.custom/development-bundle/ui/minified/jquery.effects.scale.min.js"></script>
		<script src="../libs/jquery-ui-1.8.15.custom/development-bundle/ui/minified/jquery.effects.shake.min.js"></script>-->
		<style>
			#btnCancelar, #btnCambiar{ width: 125px;  float: left;}
		</style>
		<script language ="Javascript" type = "text/javascript"> 
			$(document).ready(function(){
				$(".menu li").hover(function(){
					$(this).find('ul:first:hidden').css({visibility: "visible",display: "none"}).slideDown(400);
				},function(){
					$(this).find('ul:first').slideUp(400);
				});
				$(".menu ul.submenu ").css({display: "none"});
				$(".menu li").click(function(){
					if($(this).find('ul:first').css("display") === "block") $(this).find('ul:first').slideUp(400);
					else $(this).find('ul:first:hidden').css({visibility: "visible",display: "none"}).slideDown(400);
				});
			});
			
			function cerrarSesion(){
				$.ajax({
					type: "POST",
					url: "../app/LoginApp.php",
					dataType: "json",
					data: { accion : "logout" },
					success:function(resultado){
						if(resultado.error != true){
							if(resultado.exito == true){ document.location = '.';}
							else{ alert("Error: No se pudo cerrar la sesion");}
						}
					},
					error:function(error){alert('Error: ' + error.status);}
				});
		   }
		   
		   function cambiaPassword(){
				var errorCaptura = false;
				var passActual = $("#passActual").val();
				var passNuevo = $("#passNuevo").val();
				var passNuevoDos = $("#passNuevoDos").val();
				var idUsuario = <?php echo($_SESSION['idUsuario']);?>;
				$("#errorPassActual").text("");
				$("#errorPassNuevo").text("");
				$("#errorPassNuevoDos").text("");
				if(passActual == ""){
					errorCaptura = true;
					$("#errorPassActual").text ("Campo requerido");
					$("#errorPassActual").show("pulsate", {}, "fast");
				}
				else if(passNuevo == "" || passNuevoDos == "" || (passNuevo != passNuevoDos)){
					if(passNuevo == ""){
						errorCaptura = true;
						$("#errorPassNuevo").text("Campo requerido");
						$("#errorPassNuevo").show("pulsate", {}, "fast");
					}else if(passNuevoDos ==""){
						errorCaptura = true;
						$("#errorPassNuevoDos").text("Campo requerido");
						$("#errorPassNuevoDos").show("pulsate", {}, "fast");
					}else if(passNuevo != passNuevoDos){
						errorCaptura = true;
						$("#errorPassNuevoDos").text("Debe ser igual a nuevo password");
						$("#errorPassNuevoDos").show("pulsate", {}, "fast");
					}
				}
				if(!errorCaptura){
					$.ajax({
						type: "POST",
						url: "../app/LoginApp.php",
						dataType: "json",
						data: { accion : "cambioPassword", nick : idUsuario, password : passActual, nuevoPassword: passNuevo},
						beforeSend: function(xhr,opciones){ 
							$("#errorPassNuevoDos").text("Procesando...");
							$("#errorPassNuevoDos").show("pulsate", {}, "fast");
							$("#btnCambiar").attr("href","javascript:void(0)");
						},
						success:function(resultado){
							if(resultado.error != true){
								$("#btnCambiar").attr("href","javascript:cambiaPassword()");
								$("#errorPassNuevoDos").text("");
								if(resultado.exito == true){
									$("#passActual").val("");
									$("#passNuevo").val("");
									$("#passNuevoDos").val("");
									$("#errorPassNuevoDos").text("Listo tu nuevo password ha sido registrado");
									$("#errorPassNuevoDos").show("pulsate", {}, "fast");
								}else{
									$("#passActual").val("");
									$("#errorPassActual").text("El password actual no es el correcto");
									$("#errorPassActual").show("pulsate", {}, "fast");
								}
							}
						},
						error:function(error){alert('Error: ' + error.status);}
					});
				}
			}
		</script>
	</head>
	<body>
		<div id="contenido">
			<div id ="cabecera"> <?php include_once("cabecera.php"); ?> </div>
			<div id="contenidoCentral">
				<?php
					echo("<h3>Cambiar password</h3>");
					echo("<div id=\"divFormulario\" class=\"formulario\">");
					echo("<form>");
					echo("<table>");
					echo("<tr><td>Nick: </td><td><input type=\"text\" class=\"cajaTexto\" value=\"". $_SESSION['nickUsuario']. "\"". " readonly=\"yes\"/></td></tr>");
					echo("<tr><td>Password actual: </td><td><input id=\"passActual\" name=\"\" type=\"password\" class=\"cajaTexto\" /></td></tr>");
					echo("<tr><td></td><td id=\"errorPassActual\" class=\"errorCampo\"></td></tr>");
					echo("<tr><td>Nuevo password: </td><td><input id=\"passNuevo\" name=\"\" type=\"password\" class=\"cajaTexto\" /></td></tr>");
					echo("<tr><td></td><td id=\"errorPassNuevo\" class=\"errorCampo\" ></td></tr>");
					echo("<tr><td>Confirmar password: </td><td><input id=\"passNuevoDos\" name=\"\" type=\"password\" class=\"cajaTexto\" /></td></tr>");
					echo("<tr><td></td><td id=\"errorPassNuevoDos\" class=\"errorCampo\"></td></tr>");
					echo("<tr><td><a id=\"btnCancelar\" class=\"boton\" href=\"" . $_SERVER['HTTP_REFERER']. "\">Cancelar</a></td><td><a id=\"btnCambiar\" class=\"boton\" href=\"javascript:cambiaPassword()\">Cambiar</a></td></tr>");
					echo("</table>");
					echo("</form>Todos los campos son obligatorios<br/><br/>");
					echo("</div>");
				?>
			</div>
			<div id="columnaSecundaria"></div>
			<div id="pie"> <?php include_once("pie.php"); ?></div>
		</div>
	</body>
</html>
<?php
	}else{
		header("Location: .");
	}
?>